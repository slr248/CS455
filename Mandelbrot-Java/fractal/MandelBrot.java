...
class MandelBrot{
  ...
  public MandelBrot(int xRes,int yRes,float cxmin,float cymin,float cxmax,float cymax){
    this.xRes=xRes;
    this.yRes=yRes;
    this.cxmin=cxmin;
    this.cymin=cymin;
    ...
    for(int i=0;i<yRes;i++){
      for(int j=0;j<xRes;j++){
        ComplexNumber c=new ComplexNumber(cxmin + ((float)j)/((float)(xRes-1))*(cxmax-cxmin),cymin + ((float)i)/((float)(yRes-1))*(cymax-cymin));
  ...
  public static  void main(String[] args) throws IOException{
    ...
    MandelBrot mand=new MandelBrot(xRes,yRes,-2,-1,1,1);
    mand.displayFractal(g1);
    mand.saveFractal("C:\\Users\\sanil\\Desktop\\CS455Java\\fractal\\fractal.ppm");
    frame.pack();
